...
ENV PIP_RETRIES=10 PIP_TIMEOUT=120 PIP_DISABLE_PIP_VERSION_CHECK=1

RUN apt-get update && apt-get install -y --no-install-recommends \
        libpango-1.0-0 libpangoft2-1.0-0 libharfbuzz0b libfribidi0 \
        libcairo2 libgdk-pixbuf-2.0-0 nodejs npm \
...
COPY main.py Merged_Occupations.xlsx ./
