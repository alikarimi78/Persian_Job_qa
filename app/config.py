...
ValidationError while `app.config` is being imported — before uvicorn binds a port, and
...
`None`, an unset database gave the string `postgresql+psycopg2://None:None@None:None/None`,
...
class Settings(BaseSettings):
    ...
    # variables the engine needs are named in one place; nothing in `app/` reads them.
    ...
    def _assemble_database_url(self) -> "Settings":
        ...
        self.DATABASE_URL = (
            f"postgresql+psycopg2://{quote(self.POSTGRES_USER, safe='')}"
