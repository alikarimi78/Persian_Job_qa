...
is why the template autoescapes and `app/schemas.py:ReportIn` bounds every string.
"""
...
NOTICE = {
    "job_generated": "این شغل هنوز در پایگاه داده ثبت نشده است؛ آنچه در ادامه می‌آید "
                     "پیشنهاد سامانه است و تا تأیید مدیر، بخشی از دیتاست به شمار نمی‌رود.",
    "out_of_domain": "این پرسش خارج از دامنه‌ی مشاغل سامانه تشخیص داده شد، بنابراین "
...
def build_html(report, user, organization: str | None, unit: str | None,
    ...
    writing it into `app/reports/assets/` where the relative URLs resolve.
    """
