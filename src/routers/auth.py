...
def login(body: LoginIn, request: Request, db: Prisma = Depends(get_db)):
    """Rate limited per source *and* username (`src/rate_limit.py`), and only wrong
...
def me(user: User = Depends(get_current_user), db: Prisma = Depends(get_db)):
    ...
    the token lookup would have meant selecting the logo blob on every authenticated
    request, for the two endpoints that want the organization's *name*.
    """
