...
class EngineManager:
    ...
    def load(self, rebuild_embeddings: bool = False):
        # The one shared client, not a session of its own. That is the whole of what the
        # Prisma migration changed here: a `Session` was a unit of work that had to be
        # opened and closed around this read, while the client is a connection pool the
        # rebuild thread may use alongside the request handlers. It still reads the
        # database *after* the approving request committed, which is why
        # `routers/admin.py:approve` fires this only once `_review` has returned.
